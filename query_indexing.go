...
func (qe *QueryEngine) rebuildFileIndex() {
	...
	// Get current commit
	if headHash, err := qe.repo.refManager.GetHEAD(); err == nil && headHash != "" {
		if commit, err := qe.repo.store.GetCommit(headHash); err == nil {
			if tree, err := qe.repo.store.GetTree(commit.TreeHash); err == nil {
	...
	if qe.repo.staging != nil {
		qe.repo.staging.mu.RLock()
		for path, hash := range qe.repo.staging.files {
			if _, exists := newIndex.pathIndex[path]; !exists {
				...
				if blob, err := qe.repo.store.GetBlob(hash); err == nil {
				...
				newIndex.hashIndex[hash] = entry
			}
		}
		qe.repo.staging.mu.RUnlock()
	}
...
func (qe *QueryEngine) indexTreeFiles(tree *object.Tree, prefix string, index *FileIndex, commitHash string) {
	for _, entry := range tree.Entries {
		...
		if entry.Mode == "040000" { // Directory
			if subTree, err := qe.repo.store.GetTree(entry.Hash); err == nil {
			...
			if blob, err := qe.repo.store.GetBlob(entry.Hash); err == nil {
