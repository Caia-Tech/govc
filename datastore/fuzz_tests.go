...
func runPropertyBasedTests(t *testing.T, store DataStore) {
	...
	t.Run("PutGetInvariant", func(t *testing.T) {
		for i := 0; i < 100; i++ {
			...
			originalData := generateRandomData(1 + mathrand.Intn(10000))
			
			// Put data
			err := store.ObjectStore().PutObject(hash, originalData)
			require.NoError(t, err, "Put should succeed for valid data")
			
			// Get data
			retrievedData, err := store.ObjectStore().GetObject(hash)
	...
	t.Run("HasObjectConsistency", func(t *testing.T) {
		for i := 0; i < 50; i++ {
			hash := generateRandomHash()
			data := generateRandomData(1 + mathrand.Intn(1000))
	...
	t.Run("ObjectSizeAccuracy", func(t *testing.T) {
		for i := 0; i < 50; i++ {
			hash := generateRandomHash()
			expectedSize := mathrand.Intn(5000)
			data := generateRandomData(expectedSize)
			
			err := store.ObjectStore().PutObject(hash, data)
			require.NoError(t, err)
			
			actualSize, err := store.ObjectStore().GetObjectSize(hash)
			require.NoError(t, err)
...
func runUnicodeAndEncodingTests(t *testing.T, store DataStore) {
	...
	t.Run("EncodingNormalization", func(t *testing.T) {
		...
		err := store.ObjectStore().PutObject("single", single)
		require.NoError(t, err)
		
		err = store.ObjectStore().PutObject("composed", composed)
		require.NoError(t, err)
		
		singleRetrieved, err := store.ObjectStore().GetObject("single")
		require.NoError(t, err)
		
		composedRetrieved, err := store.ObjectStore().GetObject("composed")
		require.NoError(t, err)
...
func generateRandomHash() string {
	...
	for i := range hash {
		hash[i] = hashChars[mathrand.Intn(len(hashChars))]
	}
	return string(hash)
}
