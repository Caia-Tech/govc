...
func TestPostgresStore_ConnectionHandling(t *testing.T) {
	...
	t.Run("ValidConnection", func(t *testing.T) {
		...
		err := store.ObjectStore().PutObject("conn-test", []byte("test data"))
		assert.NoError(t, err)
		
		data, err := store.ObjectStore().GetObject("conn-test")
		assert.NoError(t, err)
...
func TestPostgresStore_TransactionIsolation(t *testing.T) {
	...
	t.Run("ReadCommittedBehavior", func(t *testing.T) {
		// Setup initial data
		err := store.ObjectStore().PutObject("read-committed-test", []byte("initial"))
		require.NoError(t, err)
	...
	t.Run("SerializableConflict", func(t *testing.T) {
		// Setup initial data
		err := store.ObjectStore().PutObject("serializable-test", []byte("initial"))
		require.NoError(t, err)
...
func TestPostgresStore_PerformanceOptimizations(t *testing.T) {
	...
	t.Run("BatchOperations", func(t *testing.T) {
		...
		start = time.Now()
		retrieved, err := store.ObjectStore().GetObjects(hashes)
		elapsed = time.Since(start)
...
func TestPostgresStore_ErrorHandling(t *testing.T) {
	...
	t.Run("DeadlockHandling", func(t *testing.T) {
		...
		err := store.ObjectStore().PutObject("deadlock-a", []byte("data a"))
		require.NoError(t, err)
		
		err = store.ObjectStore().PutObject("deadlock-b", []byte("data b"))
		require.NoError(t, err)
...
func TestPostgresStore_Monitoring(t *testing.T) {
	...
	t.Run("ObjectCounts", func(t *testing.T) {
		// Add some test data
		for i := 0; i < 10; i++ {
			err := store.ObjectStore().PutObject(fmt.Sprintf("monitor-test-%d", i), []byte("test"))
			require.NoError(t, err)
		}
	...
	t.Run("Metrics", func(t *testing.T) {
		...
		err := store.ObjectStore().PutObject("metrics-test", []byte("test"))
		require.NoError(t, err)
