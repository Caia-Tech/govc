...
func TestMemoryStore_EdgeCases(t *testing.T) {
	...
	t.Run("EmptyOperations", func(t *testing.T) {
		...
		err := store.ObjectStore().PutObjects(map[string][]byte{})
		assert.NoError(t, err)
		
		err = store.DeleteObjects([]string{})
		assert.NoError(t, err)
		
		result, err := store.ObjectStore().GetObjects([]string{})
		assert.NoError(t, err)
		...
		err = store.ObjectStore().IterateObjects("nonexistent-", func(hash string, data []byte) error {
	...
	t.Run("NilAndEmptyData", func(t *testing.T) {
		// Empty data
		err := store.ObjectStore().PutObject("empty", []byte{})
		assert.NoError(t, err)
		
		data, err := store.ObjectStore().GetObject("empty")
		assert.NoError(t, err)
		assert.Empty(t, data)
		
		size, err := store.ObjectStore().GetObjectSize("empty")
		assert.NoError(t, err)
		...
		data, err = store.ObjectStore().GetObject("nil-test")
		assert.NoError(t, err)
	...
	t.Run("ErrorConditions", func(t *testing.T) {
		// Non-existent object
		_, err := store.ObjectStore().GetObject("does-not-exist")
		assert.ErrorIs(t, err, datastore.ErrNotFound)
		
		_, err = store.ObjectStore().GetObjectSize("does-not-exist")
		assert.ErrorIs(t, err, datastore.ErrNotFound)
		
		// Delete non-existent
		err = store.ObjectStore().DeleteObject("does-not-exist")
		assert.ErrorIs(t, err, datastore.ErrNotFound)
		
		// Has non-existent
		exists, err := store.ObjectStore().HasObject("does-not-exist")
		assert.NoError(t, err)
	...
	t.Run("LargeData", func(t *testing.T) {
		...
		err := store.ObjectStore().PutObject("large", largeData)
		assert.NoError(t, err)
		
		retrieved, err := store.ObjectStore().GetObject("large")
		assert.NoError(t, err)
		assert.Equal(t, largeData, retrieved)
		
		size, err := store.ObjectStore().GetObjectSize("large")
		assert.NoError(t, err)
...
func TestMemoryStore_MetadataEdgeCases(t *testing.T) {
	...
	t.Run("UserEdgeCases", func(t *testing.T) {
		...
		user := &datastore.User{
			...
			FullName: "Test User with Üñîçödé",
			IsActive: false,
			IsAdmin:  true,
			Metadata: map[string]interface{}{
...
func TestMemoryStore_Transactions(t *testing.T) {
	...
	t.Run("TransactionIsolation", func(t *testing.T) {
		...
		_, err = store.ObjectStore().GetObject("tx-test")
		assert.ErrorIs(t, err, datastore.ErrNotFound)
		...
		data, err = store.ObjectStore().GetObject("tx-test")
		assert.NoError(t, err)
		assert.Equal(t, []byte("transaction data"), data)
	})
	...
	t.Run("TransactionRollback", func(t *testing.T) {
		// Create initial data
		err := store.ObjectStore().PutObject("rollback-test", []byte("initial"))
		require.NoError(t, err)
		...
		data, err := store.ObjectStore().GetObject("rollback-test")
		assert.NoError(t, err)
	...
	t.Run("MultipleTransactions", func(t *testing.T) {
		...
		data1, err = store.ObjectStore().GetObject("multi-tx1")
		assert.NoError(t, err)
		assert.Equal(t, []byte("data1"), data1)
		
		data2, err = store.ObjectStore().GetObject("multi-tx2")
		assert.NoError(t, err)
		assert.Equal(t, []byte("data2"), data2)
	})
}
...
func TestMemoryStore_Lifecycle(t *testing.T) {
	...
	t.Run("Info", func(t *testing.T) {
		...
		store.ObjectStore().PutObject("info-test", []byte("test"))
...
func TestMemoryStore_Metrics(t *testing.T) {
	...
	// Perform operations
	err = store.ObjectStore().PutObject("metrics-test", []byte("data"))
	assert.NoError(t, err)
	
	_, err = store.ObjectStore().GetObject("metrics-test")
	assert.NoError(t, err)
	
	err = store.ObjectStore().DeleteObject("metrics-test")
	assert.NoError(t, err)
