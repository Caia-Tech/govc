...
func runStressWorker(ctx context.Context, store DataStore, workerID int, config StressTestConfig, metrics *StressTestMetrics, initialData []string) {
	...
	for {
		...
		case <-ticker.C:
			...
			var err error
			useTransaction := mathrand.Float64() < config.TransactionPercent
			...
			if err != nil {
				...
				// Use mutex to safely update map from concurrent goroutines
				// For simplicity, we'll just use atomic operations on main counters
				// and skip the error type breakdown in concurrent access
				_ = errorType // acknowledge errorType
			} else {
...
func executeTransactionalOperation(ctx context.Context, store DataStore, workerID, opCounter int, config StressTestConfig, initialData []string) error {
	...
	shouldCommit := mathrand.Float64() < 0.9
	...
	numOps := 1 + mathrand.Intn(5) // 1-5 operations per transaction
	for i := 0; i < numOps; i++ {
		if mathrand.Float64() < config.ReadWriteRatio {
			// Read operation
			hash := initialData[mathrand.Intn(len(initialData))]
			...
			data := generateRandomDataForStress(config.MinObjectSize + mathrand.Intn(config.MaxObjectSize-config.MinObjectSize))
...
func executeSingleOperation(store DataStore, workerID, opCounter int, config StressTestConfig, initialData []string) error {
	if mathrand.Float64() < config.ReadWriteRatio {
		...
		if mathrand.Float64() < 0.8 { // 80% writes, 20% deletes
			// Write operation
