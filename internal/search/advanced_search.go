...
func NewAdvancedSearch(repo interfaces.Repository) *AdvancedSearch {
	as := &AdvancedSearch{
		repo:             repo,
...
func (as *AdvancedSearch) buildInitialIndex() {
	// Note: query engine integration disabled during refactor
	...
	for _, filePath := range files {
		...
		// to resolve the path to a hash first
		blob, err := as.repo.GetBlob(filePath) // Simplified approach
		if err != nil {
			continue
		}
		
		// Only index text files (basic heuristic)
		if as.isTextFile(filePath, blob) {
			as.IndexDocument(filePath, blob)
		}
	}
}
