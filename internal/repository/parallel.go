...
func NewRepository() *Repository {
	...
	eventBus.Start() // Start the event bus
	
	txnManager := NewAtomicTransactionManager()
	
	gc := compaction.NewGarbageCollector(createMemoryStore())
	gc.SetPolicy(compaction.CompactionPolicy{
		Enabled:            true,
		Interval:           time.Minute,
		MemoryThreshold:    0.8,
		MinObjectAge:       time.Hour,
	...
	// Initialize Query Engine after repository is created
	repo.queryEngine = search.NewQueryEngine(repo)
	
	// Initialize Delta Compression system for high-frequency commits
	repo.deltaCompression = delta.NewDeltaCompression()
	
	// Initialize Advanced Search system
	repo.advancedSearch = search.NewAdvancedSearch(repo)
...
func (pr *ParallelReality) Apply(changes interface{}) error {
	...
	case map[string][]byte:
		for path, content := range c {
			...
			pr.repo.staging.AddHash(path, hash)
		}
